...
import {Paper, Fab, Button, Box} from '@material-ui/core/';
...
function FormConstructorForm({order, values, onFieldChange, onNameChange, onBlur, onSubmit, onAddField, onDelete, onReorder, errors, validateForm, onFocus, clearError}) {
  ...
  return (
    <form onSubmit={handleSubmit}>
      <Box mb={2}>
        ...
        {order.length > MAX_FIELDS_PER_FORM ?
          null :
          <Button onClick={onAddField} variant="contained" color="primary">+ Add new field</Button>}
      </Box>
      <AppFooter>
        <Fab variant="extended" color="secondary" type="submit" disabled={!order || order.length < 1}
        > Save Form </Fab>
      </AppFooter>
    </form>
  );
